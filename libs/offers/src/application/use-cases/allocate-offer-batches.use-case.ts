...
export class AllocateOfferBatchesUseCase {
  ...
  async execute(input: {
    ...
    const ownedOffer = await this.productRepository.findOwnedOffer(
      input.offerId,
      input.requesterUserId,
    );
    if (!ownedOffer) {
      throw new NotFoundException(
        'Offer not found or does not belong to current user',
      );
    }
    ...
    const invalidItem = normalizedItems.find(
      (item) => !item.batchId || item.allocatedQuantity < 1,
    );
    if (invalidItem) {
    ...
    const uniqueBatchIds = [
      ...new Set(normalizedItems.map((item) => item.batchId)),
    ];
    if (uniqueBatchIds.length !== normalizedItems.length) {
      throw new BadRequestException(
        'Each batch can only be allocated once per request',
      );
    }
    ...
    if (batches.length !== uniqueBatchIds.length) {
      throw new BadRequestException(
        'One or more batches do not belong to the same shop and product model as the offer',
      );
    }

    const newAllocatedTotal = normalizedItems.reduce(
      (sum, item) => sum + item.allocatedQuantity,
      0,
    );
    ...
    for (const item of normalizedItems) {
      ...
      if (!ownedOffer.distributionNodeId && batch.distributionNodeId) {
        throw new BadRequestException(
          'Offers without distribution node cannot allocate distributed batches',
        );
      }

      if (
        ownedOffer.distributionNodeId &&
        batch.distributionNodeId !== ownedOffer.distributionNodeId
      ) {
        throw new BadRequestException(
          'Batches must belong to the same distribution node as the offer',
        );
      }
      ...
      if (allocatedToOtherOffers + item.allocatedQuantity > batch.quantity) {
        throw new BadRequestException(
          `Batch ${batch.batchNumber} does not have enough allocatable quantity`,
        );
      }
    }

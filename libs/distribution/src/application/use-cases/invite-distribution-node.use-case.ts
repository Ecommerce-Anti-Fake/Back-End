...
@Injectable()
export class InviteDistributionNodeUseCase {
  ...
  async execute(input: {
    ...
    if (!agentShop) {
      throw new BadRequestException('Only DISTRIBUTOR shops can be invited as child distribution nodes');
    }
    ...
    const node = await this.repository.createInvitedNode({

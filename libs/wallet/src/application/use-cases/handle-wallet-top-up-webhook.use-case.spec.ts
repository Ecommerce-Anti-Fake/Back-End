...
const webhook = (overrides: Record<string, unknown> = {}) => ({ code: '00', desc: 'success', success: true, signature: 'sig', data: { paymentLinkId: 'link-1', amount: 100000, ...overrides } });
...
describe('HandleWalletTopUpWebhookUseCase', () => {
  it('rejects an invalid provider signature', async () => {
    ...
    const useCase = new HandleWalletTopUpWebhookUseCase({} as never, {} as never, payOS as never);
  ...
  it('credits the wallet once for a successful webhook', async () => {
    ...
    const useCase = new HandleWalletTopUpWebhookUseCase(prisma as never, repository as never, { verifyWebhook: jest.fn().mockReturnValue(true) } as never);

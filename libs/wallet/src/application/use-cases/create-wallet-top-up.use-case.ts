...
export class CreateWalletTopUpUseCase {
  ...
  async execute(input: { userId: string; amount: string; idempotencyKey?: string }) {
    const idempotencyKey = input.idempotencyKey?.trim() || `WALLET_TOP_UP:${input.userId}:${Date.now()}`;
    ...
    const wallet = await this.walletRepository.findOrCreateUserWallet(input.userId, 'VND');
    const payment = await this.payOS.createPaymentLink({ amount, idempotencyKey });
    try {

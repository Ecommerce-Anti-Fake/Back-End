...
async function main() {
      ...
      await prisma.walletWithdrawal.update({
        ...
        data: {
          accountNumberEncryptedSnapshot:
            security.encryptAccountNumber(normalized),
  ...
  process.stdout.write(
    JSON.stringify({ encryptedWithdrawals: encryptedCount }),
  );
}
...
main()
  ...
  .catch((error) => {
    process.stderr.write(
      `Legacy withdrawal encryption failed: ${error instanceof Error ? error.message : 'unknown error'}\n`,
    );
    process.exitCode = 1;
  });

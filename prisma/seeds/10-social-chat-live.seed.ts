  ...
  for (let i = 0; i < COUNTS.socialComments; i += 1) {
    await prisma.socialComment.create({
      data: {
        ...
        body: i % 2 === 0 ? 'Thông tin hữu ích, mình sẽ thử quét QR khi nhận hàng.' : 'Shop có hỗ trợ kiểm tra lô sản xuất không?',
        visibility: 'PUBLIC',
  ...
  for (let i = 0; i < COUNTS.liveSessionComments; i += 1) {
    await prisma.liveSessionComment.create({
      data: {
        ...
        authorUserId: pick(ctx.users, i).id,
        body: i % 2 === 0 ? 'Mã QR này kiểm tra được tới lô sản xuất luôn hả shop?' : 'Có ưu đãi cho combo không shop?',
